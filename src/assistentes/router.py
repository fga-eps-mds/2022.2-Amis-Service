...
from .repository import AssistenteRepository
...
def create(request: AssistentesRequest, database: Session = Depends(get_database)):
    ...
    assistentes = AssistenteRepository.save(database, Assistentes(**request.dict()))
    return assistentes
...
def find_all(database: Session = Depends(get_database)):
    ...
    assistentes = AssistenteRepository.find_all(database)
    print(assistentes)
    return [AssistenteRepository.from_orm(assistente) for assistente in assistentes]
...
def find_by_cpf(cpf: str, database: Session = Depends(get_database)):
    ...
    assistente = AssistenteRepository.find_by_cpf(database, cpf)
    if not assistente:
        raise HTTPException(
...
def delete_by_id(id: str, database: Session = Depends(get_database)):
    ...
    if not AssistenteRepository.exists_by_id(database, id):
        raise HTTPException(
            status_code = status.HTTP_404_NOT_FOUND, detail="Assistente não encontrada"
        )
    AssistenteRepository.delete_by_id(database, id)
...
def update(id: str, request: AssistentesRequest, database: Session = Depends(get_database)):
    ...
    assistente = AssistenteRepository.save(database, Assistentes(id = id, **request.dict()))
    return AssistentesResponse.from_orm(assistente)